...
pub fn map_and_estimate<'py>(
    ...
    let fidelity = estimate_fidelity(&tensor, &routing, &params);
    ...
    dict.set_item("operational_fidelity", fidelity.operational_fidelity)?;
    ...
    let op_grid =
        Array2::from_shape_vec((num_layers, num_qubits), fidelity.operational_fidelity_grid)
            .expect("Shape mismatch for operational_fidelity_grid");
    dict.set_item("operational_fidelity_grid", op_grid.into_pyarray_bound(py))?;
