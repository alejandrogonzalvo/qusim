#!/usr/bin/env python3
"""
Validation benchmark: tests quadris against circuit families NOT used during model development.

Training set used: GHZ, QFT, random_circuit(seed=42) at 3/5/8/10/12 qubits.
Validation set uses: Bernstein-Vazirani, W-state, QAOA-style layers,
                     and random_circuit with DIFFERENT seeds, qubit counts (4,6,7,9),
                     and different depths (3,7,15).
"""
...
# ---------------------------------------------------------------------------
# Validation circuit generators (NONE used during model tuning)
# ---------------------------------------------------------------------------
def make_bernstein_vazirani(n, secret):
...
def make_random_circuit(n, depth, seed):
...
def generate_validation_circuits():
    circuits = []

    for n in [4, 6, 9]:
        secret = (1 << n) - 1
        circuits.append((f'bv_{n}', make_bernstein_vazirani(n, secret)))

    for n in [4, 6, 9]:
        circuits.append((f'w_{n}', make_w_state(n)))

    for n in [4, 6, 8]:
        for d in [3, 7, 15]:
            circuits.append((f'qaoa_{n}q_{d}d', make_qaoa_layer(n, d)))

    for n in [4, 6, 7, 9]:
        for d in [3, 7, 15]:
            circuits.append((f'rand_{n}q_{d}d', make_random_circuit(n, d, seed=137)))

    return circuits


# ---------------------------------------------------------------------------
# Fidelity models (ESP, Depol, quadris) — same as benchmark_live_hw.py
# ---------------------------------------------------------------------------
...
def estimate_quadris(transp, params, per_qubit):
    n = transp.num_qubits
    edges = []
    for i in range(n):
...
# ---------------------------------------------------------------------------
def main():
    parser = argparse.ArgumentParser(description='Validation benchmark (unseen circuits)')
    ...
    raw_circuits = generate_validation_circuits()
    print(f"Generated {len(raw_circuits)} validation circuits")
    ...
    if args.offline:
        ...
        saved_results_path = os.path.join(DATA_DIR, 'benchmark_validation_hw_results.json')
        with open(saved_results_path) as f:
            saved_data = json.load(f)

        hw_fidelities = []
        for meta in circuit_meta:
            match = next(c for c in saved_data['circuits']
                         if c['name'] == meta['name'] and c['num_qubits'] == meta['num_qubits'])
    ...
    for transp in transpiled:
        esp_fids.append(estimate_esp(transp, params))
        depol_fids.append(estimate_depol(transp, params))
        uniform_fids.append(estimate_quadris(transp, params, per_qubit=False))
        perqubit_fids.append(estimate_quadris(transp, params, per_qubit=True))
    ...
    # --- Plot (same 3-panel layout as benchmark_live_hw.py) ---
    ...
    ax2.set_title(f'Fidelity vs Circuit Size — {backend.name} (VALIDATION)', fontsize=15)
    ...
    plt.suptitle(f'quadris VALIDATION Benchmark — {backend.name} ({args.shots} shots)',
                 fontsize=17, y=1.02)
    plt.tight_layout()

    out_path = os.path.join(SCRIPT_DIR, 'benchmark_validation.png')
    ...
    results_path = os.path.join(DATA_DIR, 'benchmark_validation_hw_results.json')
